...
public class Sudoku : MonoBehaviour
{
    ...
    bool Solve(Matrix<int> matrix,int x, int y,List<Matrix<int>> solution)
    {
        watchdog--;
        ...
        else
        {
            for (int value = 1; value <= 9; value++)
            {
                ...
                Matrix<int> aux = matrix.Clone();
                aux[x, y] = value;
                solution.Add(aux);
               
                if (CanPlaceValue(matrix, value, x, y))
                {
                  
                    if (Solve(aux, x + 1, y,solution))
                    {
                       
                        return true;
                    }
                    matrix[x, y] = _board[x, y].number = 0;
                }
            }
        }
        return false;
    ...
    void SolvedSudoku()
    {
        ...
        watchdog = 100000;
        var result = Solve(_createdMatrix, 0, 0,solution);
    ...
    void CreateSudoku()
    {
        ...
        var result = Solve(_createdMatrix, 0, 0,l); ;
    ...
    void CreateNew()
    {
        _createdMatrix = new Matrix<int>(Tests.validBoards[Random.Range(1,25)]);
        TranslateAllValues(_createdMatrix);
    }
